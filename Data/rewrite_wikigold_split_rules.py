        ...
        for variant in variants:
            for size in sizes:
                for i in repetition: #or list
                    #Train, Test, Valid
                    file[43] = '    "H:\\\\My Files\\\\School\\\\Grad School WLU\\\\MRP\\\\Research\\\\Files\\\\Data\\\\Textfiles\\\\Rule_'+str(rate)+"\\\\"+variant+'\\\\'+str(size)+'\\\\v'+str(i)+'_Augmented.txt"'
                    ...
                    with open("H:\\My Files\\School\\Grad School WLU\\MRP\\Research\\Files\\Data\\Textfiles\\Rule_"+str(rate)+"\\\\"+variant+'\\\\'+str(size)+'\\\\0'+str(rate)[2]+variant+str(size)+"v"+str(i)+"_wikigold_split.py","w+") as f2:
                        f2.write(''.join(file))
