...
for index, row in df.iterrows():
    ...
    for i,entry in enumerate(tag_list):
        if entry != "O":
...
######FURTHER TAG REPLACEMENT TESTING
df = gold_dataframe()
test1 = df["Article"][0]
...
for i,sentence in enumerate(tag_sentences):
    for j,tag in enumerate(sentence):
        if tag!="O":
            word_sentences[i][j] = tag
            
print(word_sentences[0])
print(tag_sentences[0])

testing = " ".join(word_sentences[0])
testing = 'I-MISC is the tenth album from I-MISC Punk Techno band I-ORGI-ORGI-ORGI-ORG .'
